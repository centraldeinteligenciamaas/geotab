-- ============================================================================
-- grupo_id: chave numérica para o relacionamento  — 2026-08-26
--
-- PROBLEMA: `todos_grupos` (texto ~126 bytes) como chave do relacionamento
-- pesa 635 MB numa view de 5,3M linhas — 25% dos 2,5 GB que o Power BI tenta
-- puxar num único stream. É o que produz
--   "DataSource.Error: PostgreSQL: Exception while reading from stream"
-- (o log do servidor confirma: FATAL: conexão com cliente foi perdida).
--
-- SOLUÇÃO: chave surrogate `grupo_id` = hashtext(todos_grupos), 4 bytes.
--   126 bytes -> 4 bytes = 31x menor.
--   Verificado: 1.721 grupos -> 1.721 hashes distintos, ZERO colisão.
--   hashtext é determinístico, então fato e dimensão sempre casam.
--
-- ONDE:
--   * grupo_id entra em TODAS as views (é a nova chave do relacionamento).
--   * `todos_grupos` PERMANECE nas views pequenas (< 200k linhas), onde o custo
--     é irrelevante e o Power BI atual ainda o referencia.
--   * `todos_grupos` SAI de vw_saneago_relatorio_viagens (5,3M linhas): -635 MB.
--   * `veiculo` também SAI da viagens (-179 MB): é redundante com
--     placa + modelo_padrao, e era ele que exigia os ReplaceValue FITA/FIAT.
--   Total: ~814 MB a menos no maior stream.
--
...
WITH (security_invoker = on) AS
 SELECT e.device_id AS id,
    c.serial, c.placa, c.todos_grupos, c.grupo_id,
    e.dia AS data,
  ...
  GROUP BY e.device_id, c.serial, c.placa, c.todos_grupos, c.grupo_id, e.dia, o.odometro, o.odometro_gps;

-- ----------------------------------------------------- relatório viagens ---
-- SEM `todos_grupos` (-635 MB) e SEM `veiculo` (-179 MB). Use grupo_id p/ o
-- relacionamento e modelo_padrao p/ o modelo. SEM ORDER BY (ver views.sql).
...
WITH (security_invoker = on) AS
 WITH base AS (
   SELECT r.device_id, r.ano, r.mes, r.km, r.duracao_segundos, r.dias_utilizados, r.viagens,
          c.placa, c.marca, c.modelo, c.todos_grupos, c.grupo_id, c.marca_padrao, c.modelo_padrao,
 ...
 SELECT placa, marca, modelo, todos_grupos, grupo_id, ano, mes,
...
WITH (security_invoker = on) AS
 WITH base AS (
   SELECT r.device_id, r.ano, r.mes, r.km, r.duracao_segundos, r.dias_utilizados,
     ...
     FROM tb_resumo_mensal r JOIN vw_saneago_cadastro c ON c.id = r.device_id
    WHERE make_date(r.ano, r.mes, 1) <= CURRENT_DATE)
 SELECT todos_grupos, grupo_id, ano, mes,
        to_char(make_date(ano, mes, 1), 'YYYY-MM') AS ano_mes,
...
CREATE VIEW vw_saneago_grupos AS
 ...
 SELECT hashtext(todos_grupos) AS grupo_id,   -- CHAVE do relacionamento
