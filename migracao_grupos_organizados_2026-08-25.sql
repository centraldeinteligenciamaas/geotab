-- ============================================================================
-- Organização final das colunas de grupo  (2026-08-25)
--
-- vw_saneago_grupos (DIMENSÃO):
--   todos_grupos_original  texto cru da Geotab (com Vehicle/Ethanol/Diesel/...)
--   todos_grupos           tratado: só os grupos (SUP_/REG_/ULOT_/OPE_/...)  <- CHAVE
--   sup_codigo  / sup_nome  / sup_cod_nome
--   reg_codigo  / reg_nome  / reg_cod_nome
--   ulot_codigo / ulot_nome / ulot_cod_nome
--   operacao, outros        níveis restantes ("e etc"), sem quebra por código
--
-- TODAS as demais views: SOMENTE `todos_grupos` (tratado).
--   Removidas: operacao, sup, reg, ulot, outros, sup_oficial (cadastro/resumo)
--              lotacao, regional, superintendencia, superintendencia_oficial
--              (motoristas) e as colunas *_codigo/*_nome/*_cod_nome.
--   A hierarquia agora vem do relacionamento todos_grupos -> vw_saneago_grupos.
--
-- MANTIDO de propósito: `grupo` / `uo_lotacao` (a UO de lotação exibida no
--   painel nas abas Produtividade e Viagens). Não é a lista de grupos, é um
--   campo de negócio já usado nos visuais — removê-lo quebraria a tela.
--   Se quiser tirar também, é um DROP+CREATE simples.
--
-- Views são derrubadas e recriadas (CREATE OR REPLACE não remove coluna).
...
-- ---------------------------------------------------------------- grupos ---
-- DIMENSÃO: única view com a quebra por nível.
-- UNIÃO de veículos + motoristas (sem os motoristas, 81% deles ficariam órfãos).
-- CHAVE ÚNICA: vários textos originais colapsam na mesma chave tratada (diferem
-- só nos rótulos ignorados ou na ORDEM deles) — 2.116 originais -> 1.721 chaves.
-- Como o Power BI exige chave única na dimensão (relacionamento muitos-p/-um),
-- agrupamos por todos_grupos e mostramos UM original representativo (min).
-- Os níveis (sup/reg/ulot) são idênticos entre os originais de um mesmo grupo,
-- porque só dependem dos tokens que sobrevivem à limpeza.
