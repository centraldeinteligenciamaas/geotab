...
- `tb_comportamento_motorista` (2026-06-18) — MESMOS eventos, mas por motorista/device/dia/tipo (captura `ev["driver"]` no `processar()`, antes descartado). device_id é a CHAVE de ligação com tb_comportamento_eventos. Só eventos com motorista identificado (~40-57%). Janela 6 meses (limpeza própria). Base do score em vw_motoristas. Backfill 6m via env `COMPORTAMENTO_BACKFILL=1` (one-shot; força backfill mesmo com buckets existentes — re-upsert idempotente dos device buckets + preenche os por motorista).
...
- `vw_motoristas` (2026-06-18) — por MOTORISTA, ano 2026: `lotacao`/`regional`/`superintendencia` (JOIN tb_motoristas por id=motorista_id), veículos conduzidos (`veiculos` string_agg de placas, `qtd_veiculos`), viagens, km, horas movimento/ocioso/parado, contadores de eventos e `score_seguranca`. SCORE = método "Event Count" do Geotab Driver Safety Scorecard (validado por pesquisa): por categoria `100 - eventos*1000/km` (piso 0), nota = MÉDIA das 4 categorias (0-100, maior=mais seguro; menor=mais risco). Difere do portal se ele usar método Hybrid (pondera severidade/distância em violação, não armazenadas). Fonte eventos: tb_comportamento_motorista; fonte km/horas: tb_viagens.
